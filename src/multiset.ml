...
module L : Multiset = struct
  ...
  let add ?(cnt = 1) e l =
    let rec repeat c l = if c == 0 then l else e :: repeat (c - 1) l in
    let rec ins = function
      | [] -> repeat cnt []
      | e' :: l' as l ->
          let c = compare e.tag e'.tag in
          if c <= 0 then repeat cnt l else e' :: ins l'
    in
    if cnt <= 0 then
      raise
        (Invalid_argument
           "Multiset.add: optional parameter 'c' must be strictly positive.")
    else ins l
  ...
  let rec remove e = function
    ...
    | e' :: l' as l ->
        let c = compare e.tag e'.tag in
        if c == 0 then l' else if c < 0 then l else e' :: remove e l'
  ...
  let rec equal l1 l2 =
    match (l1, l2) with
    | [], [] -> true
    | e1 :: l1', e2 :: l2' -> e1.tag == e2.tag && equal l1' l2'
    | _, _ -> false
...
module R : Multiset = struct
  ...
  let add ?(cnt = 1) k t =
    let rec ins = function
      | Empty -> Leaf (1, k)
      | Leaf (i, j) as t ->
          if j.tag == k.tag then Leaf (i + cnt, j)
  ...
  let remove k t =
    let rec rmv = function
      | Empty -> Empty
      | Leaf (i, j) ->
          if j.tag == k.tag then if i == 1 then Empty else Leaf (i - 1, j)
          else t
      | Branch (p, m, t0, t1) as t ->
          if match_prefix k.tag p m then
  ...
  let rec subset t1 t2 =
    ...
    | _, Leaf (i2, k2) -> (
        match mem k2 t1 with Some i1 -> i2 <= i1 | None -> false)
    | Branch (p1, m1, l1, r1), Branch (p2, m2, l2, r2) ->
